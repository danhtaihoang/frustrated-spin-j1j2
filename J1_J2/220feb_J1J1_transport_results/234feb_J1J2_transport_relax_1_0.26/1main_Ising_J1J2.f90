...
!----------------------------------------------------------------------------------------------------!
!     PROGRAMME: MONTE CARLO TRANSPORT OF SPINS - BCC and SC
!     Chinh sua ngay 10/6/2010: Tinh Mz_F1,Mz_AF1,Mz_F2,Mz_AF2,Mz_SC, ....
!     Chinh sua ngay 20/10/2010: Tim GS (T=0) tai cac gia tri J2 khac nhau, muc dich tim ra gia tri toi han
!                                cua J2 = - 0.25
!     Chinh sua ngay 21/10/2010: Bo sung chuong trinh histogram energy
!!    17.02.2012: Bo sung config GS1 va cach tinh M cho GS1 (doi voi absJ2<0.25), phase diagram    
!     20.02.2012: Sua lai de chay transport voi time relaxation       
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
...
!*==*===*==*==*==*==*==*==*===*==*==*==*==*==*==*===*==*==*==*==*==*==*===*==*==*==*==*==*==*==*==*== 
      ...
      CHARACTER (256)       Ligne11,Ligne121,Ligne13,Ligne14
      ...
      INTEGER (KIND=4),PARAMETER :: n_atom_p_m=1
      INTEGER (KIND=4),PARAMETER :: n_value_Mz=2

      REAL    (KIND=8),PARAMETER :: nul=0.  

      INTEGER (KIND=4) :: saveconf,savedata,bugvar,case_histo,case_histo_O,case_histo_P
      ...
      INTEGER (KIND=4) :: i_average_thermal,n_average_thermal
      ...
      REAL    (KIND=8) :: aaa,x,y,z,px,py,pz,rdm_vit1,fieldB,N0,Vsph,Vbox
      ...
      REAL    (KIND=8) :: T,dT,spin_tmp,energy,energy_moy,energy2,energy2_moy,Cv,Ksi,Ksi2
      ...
      REAL    (KIND=8) :: J3_stmp,J31_stmp,J32_stmp,J3v,J3s,time_relax,Tc
...
!Ouverture du fichier de sortie average
      ...
      OPEN(unit=123,file='EN_recherche_gs.dat')
      OPEN(unit=200,file='time_relax.dat')

!Appel de la routine d initialisation du generateur aleatoire
...
!*==*===*==*==*==*==*==*==*===*==*==*==*==*==*==*===*==*==*==*==*==*==*===*==*==*==*==*==*==*==*==*==*!
      ...
      ENDDO  
                                                     !1::FIN i_T::!
!Desallocation dynamique des tableaux et fermeture des fichiers de sorties       
...
!_____________________________________________________________________________________________________
      ...
      READ(11, '(A50)')         tamp
      READ(11, '(A30,(I8))')    tamp, saveconf
      READ(11, '(A30,(I8))')    tamp, savedata
      READ(11, '(A50)')         tamp
      READ(11, '(A50)')         tamp
...
!!!=================================================================================================
      ...
      n_average_transport=100000/(n_equi_elec_2+n_transport_elec)      ! n_average_transport      
      n_equi_reseau_2=50000/n_average_transport+5                     ! n_equi_reseau_2 (3)
...
!              Initialisation du generateur de nombre aletaoire en fonction de
...
!_____________________________________________________________________________________________________
      ...
      DO k=1,natz
            DO j=1,naty
                  DO i=1,natx
                        DO l=1,n_atom_p_m                               
                              tab_spin_a(i,j,k,l)=(-1.)**mod(i+j+k,2)
      ...
      DO k=1,natz
            DO j=1,naty
                  DO i=1,natx
                        DO l=1,n_atom_p_m                               
                              tab_spin_a(i,j,k,l)=(-1.)**mod(i+j,2)
                              !tab_spin_a(i,j,k,l)=1.
...
!              N passage sur tous les atomes de la structure. ________________________________________
!_____________________________________________________________________________________________________
...
!_____________________________________________________________________________________________________
      ...
      DO i_equi_reseau_2=1,n_equi_reseau_2
            CALL equi_atom()
      ENDDO
...
!=====================================================================================================
!__ SUBROUTINE Rethermalisation pour transport des electrons ________________________________________
!_____________________________________________________________________________________________________
...
!              Passage sur tous les atomes de la structure et critere 
            ...
            DO j=1,naty
                  ...
                  DO i=1,natx
                        ...
                        EN_tmp1=-tab_spin_a(i,j,k,1) &
                                    *(J11_stmp*(tab_spin_a(i,j,k,2)+tab_spin_a(i,jm,k,2)&
                                           +tab_spin_a(im,jm,k,2)+tab_spin_a(im,j,k,2))&
                                    +J12_stmp*(tab_spin_a(i,j,km,2)+tab_spin_a(i,jm,km,2)&
                                           +tab_spin_a(im,jm,km,2)+tab_spin_a(im,j,km,2))&

                                    +J2_stmp*(tab_spin_a(im,j,k,1)+tab_spin_a(i,jp,k,1)&
                                             +tab_spin_a(ip,j,k,1)+tab_spin_a(i,jm,k,1))&
                                    +J21_stmp*tab_spin_a(i,j,kp,1)&
                                    +J22_stmp*tab_spin_a(i,j,km,1)&
                              
                                    +J3_stmp*(tab_spin_a(ip,jp,k,1)+tab_spin_a(ip,jm,k,1)&
                                              +tab_spin_a(im,jm,k,1)+tab_spin_a(im,jp,k,1))&
                                    +J31_stmp*(tab_spin_a(i,jp,kp,1)+tab_spin_a(ip,j,kp,1)&
                                               +tab_spin_a(i,jm,kp,1)+tab_spin_a(im,j,kp,1))&
                                    +J32_stmp*(tab_spin_a(i,jp,km,1)+tab_spin_a(ip,j,km,1)&
                                               +tab_spin_a(i,jm,km,1)+tab_spin_a(im,j,km,1)))


                        CASE(2)

                        EN_tmp1=-tab_spin_a(i,j,k,2) &
                                 *(J11_stmp*(tab_spin_a(i,j,k,1)+tab_spin_a(i,jp,k,1)&
                                            +tab_spin_a(ip,jp,k,1)+tab_spin_a(ip,j,k,1))&
                                  +J12_stmp*(tab_spin_a(i,j,kp,1)+tab_spin_a(i,jp,kp,1)&
                                            +tab_spin_a(ip,jp,kp,1)+tab_spin_a(ip,j,kp,1)))
                        END SELECT
...
!!!! =================================================================================================
!!!! =================================================================================================
!Mz(1) la Mz_F1; Mz(2) la Mz_AF1; Mz(3),M(4),M(5) la Mz_SC cua tung truong hop, ,
                        IF (l==1) THEN
                              Mz(1)=Mz(1) + ((-1.)**mod(i+j+k,2))*tab_spin_a(i,j,k,1)
                              Mz(2)=Mz(2) + ((-1.)**mod(i+j,2))*tab_spin_a(i,j,k,1)
                        END IF  
...
!! Khai bao cac bien cho Histogram      
      INTEGER,PARAMETER :: n_histo = 101
      REAL,PARAMETER :: EN_min_histo = -1.50
      REAL,PARAMETER :: EN_max_histo = -0.90

      INTEGER :: i_histo
      REAL :: del_EN_histo
      REAL,DIMENSION(n_histo) :: EN_histo,P_histo

      OPEN(unit=130,file='histogram.dat')
...
!! END khai bao bien cho histogram
      ...
      DO i_average_thermal=1,n_average_thermal
            ...
            CALL value_thermal()           
            DO i=1,n_value_Mz
                  Mz_moy(i)=Mz_moy(i)+Mz(i)                
            END DO
            energy_moy=energy_moy+energy
            energy2_moy=energy2_moy+energy2
            Mz_2_moy(1)=Mz_2_moy(1)+Mz_2(1)
            Mz_2_moy(2)=Mz_2_moy(2)+Mz_2(2)

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      
            !!! ----------------- BEGIN program histogram
            
            del_EN_histo = (EN_max_histo-EN_min_histo)/real(n_histo-1)
      ...
      DO i_histo=1,n_histo
            EN_histo(i_histo)=EN_min_histo+real(i_histo-1)*del_EN_histo
            WRITE(130,*)i_histo, EN_histo(i_histo),P_histo(i_histo)
      ENDDO     
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      DO i=1,n_value_Mz  
            Mz_moy(i)=Mz_moy(i)/real(n_average_thermal)
      END DO
      ...
      Mz_2_moy(2)=Mz_2_moy(2)/real(n_average_thermal)

      Cv =real(n_atom)*(energy2_moy-energy_moy**2.)/(T**2.)
      Ksi=real(n_atom)*(Mz_2_moy(1)-Mz_moy(1)**2.)/T
      Ksi2=real(n_atom)*(Mz_2_moy(2)-Mz_moy(2)**2.)/T

      WRITE(Ligne121,*) T,energy_moy,Mz_moy(1),Cv,Ksi,Mz_moy(2),Ksi2
...
!=====================================================================================================
!__ SUBROUTINE lecture conf3D transport : 
!              Lecture du fichier de structure atome 3D + electron 3D. _______________________________
!_____________________________________________________________________________________________________
...
!Lecture du fichier
      OPEN(unit=11,file='bank_config/struct_3D_thermalisation'//trim(name)//'.pdb')
...
!              Lecture du fichier de structure atome 3D + electron 3D. _______________________________
!_____________________________________________________________________________________________________
      SUBROUTINE save_conf3D_thermal()
...
!Ecriture du fichier config_ incrémenté
      OPEN(unit=11,file='bank_config_out/struct_3D_thermalisation'//trim(name)//'.pdb')
      ...
      END SUBROUTINE save_conf3D_thermal
...
!Ecriture du fichier config_ incrémenté
      OPEN(unit=11,file='config_out/conf_3D_thermalisation'//trim(name)//'.pdb')
      !---------------------------- Bieu dien cac atom xung quanh
      DO k=1,natz
            DO j=1,naty
                  DO i=1,natx
                        x=real(i-1)*aaa
                        ...
                        IF (int(tab_spin_a(i,j,k,1))==1) THEN 
                              spinAt='Au'
            ...
            ENDDO
      ENDDO
      !-------------------------Bieu dien cac atom o trung tam
      DO k=1,natz-1
            DO j=1,naty-1
                  DO i=1,natx-1
                        x=real(i-1)*aaa+aaa/2.
                        y=real(j-1)*aaa+aaa/2.
                        z=real(k-1)*aaa+aaa/2.

                        IF (int(tab_spin_a(i,j,k,2))==1) THEN 
                              spinAt='Au'
                              WRITE(11,'("ATOM",9X,A3,14X,3F8.3,1X,"1.00",1X,F8.3)') &
                                   spinAt,x,y,z,nul  
                        END IF
...
!!====================================================================================================
!!====================================================================================================
!__ SUBROUTINE recherche_gs
!              Tim GS cua he __________________________________________________
!_____________________________________________________________________________________________________

      SUBROUTINE recherche_gs()

      IMPLICIT NONE

      INTEGER (KIND=4) :: i_recherche_gs,compt_i
      REAL (KIND=8) :: H_tmp1,EN_recherche_gs

      compt_i=0

      WRITE(*,*)'Thuc hien tim GS'

!!===========================================================
!!!! BEGIN configuration for program recherche GS
!!===========================================================
      DO k=1,natz
            !km=k-1
            !kp=k+1
      ...
      ENDDO      
!!===========================================================
!!!! END configuration for program recherche GS
!!===========================================================
      ...
      DO i_recherche_gs=1,10000

      EN_recherche_gs=0.

!!===========================================================
!!!! BEGIN Aligner le spin parallel a son champ local
!!===========================================================
      ...
      DO k=1,natz
            ...
            DO j=1,naty
                  ...
                  DO i=1,natx
                        ...
                        H_tmp1=-J11_stmp*(tab_spin_a(i,j,k,2)+tab_spin_a(i,jm,k,2)&
                                           +tab_spin_a(im,jm,k,2)+tab_spin_a(im,j,k,2))&
                                    +J12_stmp*(tab_spin_a(i,j,km,2)+tab_spin_a(i,jm,km,2)&
                                           +tab_spin_a(im,jm,km,2)+tab_spin_a(im,j,km,2))&
                        ...
                        !tab_spin_a(i,j,k,l)=H_tmp1/(abs(H_tmp1))
      ...
      ENDDO      
!!====================================================
!!!! END Aligner le spin parallel a son champ local
!!====================================================     

!!====================================================
!!!! BEGIN cacul energy du reseau
!!==================================================== 
      ...
      DO k=1,natz
            ...
            DO j=1,naty
                  ...
                  DO i=1,natx
                        ...
                        EN_recherche_gs = - tab_spin_a(i,j,k,1)*H_tmp1

                  ENDDO
            ENDDO
      ENDDO      
      
      WRITE(123,*)i_recherche_gs,EN_recherche_gs

!!=====================================================
!!!! END cacul energy du reseau
!!=====================================================
      END DO

!!====================================================
!!!! BEGIN xem hinh dang spin de tim GS
!!==================================================== 

      OPEN(unit=124,file='gs.pdb')
      OPEN(unit=125,file='gs.dat')
      ...
      DO k=1,natz
            DO j=1,naty
                  DO i=1,natx
                        ...
                        IF (int(tab_spin_a(i,j,k,1))==1) THEN 
                              spinAt='H'
                              WRITE(124,'("ATOM",9X,A3,14X,3F8.3,1X,"1.00",1X,F8.3)') &
                                   spinAt,x,y,z,nul  
                        END IF
                        ...
                        END IF                                
                        compt_i=compt_i+1
                        WRITE(125,*)i-1,j-1,k-1,compt_i,tab_spin_a(i,j,k,1)
                  ENDDO
            ENDDO
      ENDDO
      ...
      CLOSE(125) 
!!====================================================
!!!! END xem hinh dang spin de tim GS
!!==================================================== 
  
      END SUBROUTINE recherche_gs
...
!_____________________________________________________________________________________________________
      ...
      !initialisation du tableau electron dans num_maille de la structure
      tab_maille_e(:,:)=0
      tab_maille_a(:,:)=0
            ...
            DO j=1,naty
                  DO i=1,natx
                        ...
                        DO l=1,n_atom_p_m
                              ...
                              tab_pos_a(i_a,2)=y+aaa/2.   
                              tab_pos_a(i_a,3)=z+aaa/2. 
...
!genere aleatoirement les positions
                  ...
                  px=px*aaa*real(natx)
                  py=py*aaa*real(naty)
                  pz=pz*aaa*real(natz-0.5)

!Remplissage tableau position et spin des electron itinerants
                  tab_pos_e(i_e,1)=px 
                  tab_pos_e(i_e,2)=py 
                  tab_pos_e(i_e,3)=pz 
                  tab_pos_e(i_e,4)=1.

!Appel du remplissage tableau maille voisines
                  gam_new=-1
...
!!Positionnement de l electron dans le tableau
            alp=int(px/aaa)+1     
            bet=alp+natx*int(py/aaa)
            bcnm=bet+natx*naty*int(pz/aaa)
...
!              Rechargement du reseau (uniquement) apres rethermalisation de la
...
!Calcul des vitesse sur chaque composante d espace
...
!Conversion de la position de l electron en numero de maille
      ...
      bet=alp+natx*int(tab_pos_e(i_e,2)/aaa)
      gam_old=bet+natx*naty*int(tab_pos_e(i_e,3)/aaa)

      pix=int(tab_pos_e(i_e,1)/aaa)
      piy=int(tab_pos_e(i_e,2)/aaa)
      piz=int(tab_pos_e(i_e,3)/aaa)
      prx=real(pix)*aaa
      pry=real(piy)*aaa
      prz=real(piz)*aaa
...
!-------------------------------------------------------------------------------
                  IF((prz+real(bcz)*aaa >= 0.).and.(prz+real(bcz)*aaa < (real(natz)-0.5)*aaa))THEN
...
!-------------------------------------------------------------------------------
      IF((tab_pos_e_tmp(3) < (real(natz)-0.5)*aaa) .and. tab_pos_e_tmp(3) >= 0)THEN
                tab_pos_e_tmp(3) =  tab_pos_e_tmp(3)                
      ELSE IF(tab_pos_e_tmp(3) >= (real(natz)-0.5)*aaa)THEN
                tab_pos_e_tmp(3) = -tab_pos_e_tmp(3)+2.*(real(natz)-0.5)*aaa
...
!Condition periodique sur y
!-------------------------------------------------------------------------------
      IF((tab_pos_e_tmp(2) < real(naty)*aaa) .and. tab_pos_e_tmp(2) >= 0)THEN
                tab_pos_e_tmp(2) =  tab_pos_e_tmp(2)
      ELSE IF(tab_pos_e_tmp(2) >= real(naty)*aaa)THEN
                tab_pos_e_tmp(2) =  tab_pos_e_tmp(2)-real(naty)*aaa
...
!Conversion de la position de l electron en numero de maille
      ...
      bet=alp+natx*int(tab_pos_e_tmp(2)/aaa)
      gam_new=bet+natx*naty*int(tab_pos_e_tmp(3)/aaa)
...
!----------------------------------------------------------------------------------------------
      ...
      piy=int(tab_pos_e_tmp(2)/aaa)
      piz=int(tab_pos_e_tmp(3)/aaa)
      prx=real(pix)*aaa
      pry=real(piy)*aaa
      prz=real(piz)*aaa
...
!Repositionnement electron non deplace (sur axe y)
      IF((tab_pos_e(i_e,2)>pvy).and.(abs(tab_pos_e(i_e,2)-pvy)>real(naty)*aaa/2.))THEN
            pvy=tab_pos_e(i_e_tmp,2)+real(naty)*aaa
      ELSE 
      IF((tab_pos_e(i_e,2)<pvy).and.(abs(tab_pos_e(i_e,2)-pvy)>real(naty)*aaa/2.))THEN
            pvy=tab_pos_e(i_e_tmp,2)-real(naty)*aaa
      END IF
...
!Repositionnement electron deplace (sur axe y)
      IF((tab_pos_e_tmp(2)>pvy).and.(abs(tab_pos_e_tmp(2)-pvy)>real(naty)*aaa/2.))THEN
            pvy=tab_pos_e(i_e_tmp,2)+real(naty)*aaa
      ELSE 
      IF((tab_pos_e_tmp(2)<pvy).and.(abs(tab_pos_e_tmp(2)-pvy)>real(naty)*aaa/2.))THEN
            pvy=tab_pos_e(i_e_tmp,2)-real(naty)*aaa
...
!Repositionnement electron non deplace (sur axe y)
      IF((tab_pos_e(i_e,2)>pvy).and.(abs(tab_pos_e(i_e,2)-pvy)>real(naty)*aaa/2.))THEN
            pvy=tab_pos_a(i_a,2)+real(naty)*aaa
      ELSE 
      IF((tab_pos_e(i_e,2)<pvy).and.(abs(tab_pos_e(i_e,2)-pvy)>real(naty)*aaa/2.))THEN
            pvy=tab_pos_a(i_a,2)-real(naty)*aaa
      END IF
...
!              voisines de sa nouvelle et ancienne maille. ___________________________________________
!_____________________________________________________________________________________________________
...
!Conversion de la position de l electron en numero de maille
      alp=int(tab_pos_e(i_e,1)/aaa)+1     
      bet=alp+natx*int(tab_pos_e(i_e,2)/aaa)
      gam=bet+natx*naty*int(tab_pos_e(i_e,3)/aaa)
...
!---------------------------------------------------------------------------------------------------
! SUBROUTINE equi_elec_2(): Tao lai trang thai equilibre cua tat ca electron sau moi lan trasport
